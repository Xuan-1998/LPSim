  ...
  if (peopleFile.open(QIODevice::ReadWrite | QIODevice::Truncate)) {
    ...
    for (int p = 0; p < trafficPersonVec.size(); p++) {
      ...
      streamP << "," << trafficPersonVec[p].avg_speed[1];
			...
			streamP << "," << trafficPersonVec[p].prevEdge[0];
			streamP << "," << trafficPersonVec[p].prevEdge[1];
			streamP << "," << trafficPersonVec[p].prevEdge[2];
			streamP << "," << trafficPersonVec[p].prevEdge[3];
			streamP << "," << trafficPersonVec[p].prevEdge[4];
			streamP << "," << trafficPersonVec[p].prevEdge[5];
			streamP << "," << trafficPersonVec[p].prevEdge[6];
