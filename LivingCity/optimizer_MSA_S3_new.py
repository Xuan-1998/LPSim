...
BEST_PARAMS = {
    ...
    "THETA2_DELAY_WEIGHT": 0.01,  # Using a reasonable default
    "LEARNING_RATE": 0.05,
...
def update_tolls_tunable_MSA(iter_df: pd.DataFrame, params: dict) -> pd.Series:
    ...
    theta2 = params["THETA2_DELAY_WEIGHT"]
...
def run_single_experiment(exp_params: dict, MAX_ITERATIONS):
    ...
    for i in range(MAX_ITERATIONS):
        ...
        objective_type = exp_params.get("objective_type", "flow_weighted_S3")

        if objective_type == "flow_weighted_S3":
            revenue = (iter_df['toll_fee'] * iter_df['final_flow_hv'] +
                       exp_params["AV_TOLL_DISCOUNT"] * iter_df['toll_fee'] * iter_df['final_flow_av']).sum()
        ...
        else:  # lane_weighted for S1 and S2
            revenue = (iter_df['toll_fee'] * (iter_df['final_flow_hv'] + iter_df['final_flow_av'])).sum()
            # Use flow-weighted average travel time for congestion term
            total_flow = iter_df['final_flow_hv'] + iter_df['final_flow_av']
            weighted_time_num = (
                        iter_df['final_travel_time_hv'] * iter_df['final_flow_hv'] + iter_df['final_travel_time_av'] *
                        iter_df['final_flow_av'])
            avg_travel_time = np.divide(weighted_time_num, total_flow,
                                        out=np.zeros_like(weighted_time_num, dtype=float), where=(total_flow != 0))
            total_delay = (iter_df['lanes'] * (avg_travel_time - iter_df['free_flow_time'])).sum()

        objective = (exp_params["THETA1_REVENUE_WEIGHT"] * revenue) - (exp_params["THETA2_DELAY_WEIGHT"] * total_delay)
